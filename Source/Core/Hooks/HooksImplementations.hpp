...
{
	spdlog::info("HkLocalPlayerStart called: this={:p}", pLocalPlayer);
...
void __fastcall HooksDefinitions::HkNetworkPlayerStart(void* pNetworkPlayer, const void* methodInfo)
{
	spdlog::info("HkNetworkPlayerStart called: this={:p}", pNetworkPlayer);
...

	{
		...
		{
			auto* localPlayerScript = netPlayer->localPlayerScript;
			if (localPlayerScript)
			{
				gpCtx->localPlayer = localPlayerScript;
				spdlog::info("*** DETECTED LOCAL PLAYER *** {:p}", (void*)localPlayerScript);
			}
		}
	}

	spdlog::info("  players count={}", gpCtx->players.size());

	return HooksDefinitions::oNetworkPlayerStart(pNetworkPlayer, methodInfo);
}
void __fastcall HooksDefinitions::HkNetworkPlayerDestroy(void* pNetworkPlayer)
{
	spdlog::info("HkNetworkPlayerDestroy called: this={:p}", pNetworkPlayer);
...
HRESULT __stdcall HooksDefinitions::HkSwapChainPresent(IDXGISwapChain* pSwapChain, UINT syncInterval, UINT flags)
{
	...
	if (context.IsImGuiInited())
	{
		if (gpCtx)
		{
			std::lock_guard<std::mutex> lock(gpCtx->mtx);
			gpCtx->localPlayer = nullptr;
			for (auto* np : gpCtx->players)
			{
				if (np && np->localPlayerScript)
				{
					gpCtx->localPlayer = np->localPlayerScript;
					break;
				}
			}
		}
